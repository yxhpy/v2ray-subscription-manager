...
# 自定义测速参数
./v2ray-subscription-manager speed-test-custom https://your-subscription-url --concurrency=5 --timeout=20
```
...
### 测速工作流命令
```bash
speed-test <订阅链接>                # 默认配置测速
speed-test-custom <订阅链接> [选项]   # 自定义测速
  选项:
    --concurrency=数量              # 并发数（默认3）
